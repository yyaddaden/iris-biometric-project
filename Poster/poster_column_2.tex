\block{}{
	...
	\vspace{-0.5em}
	La \textbf{GLCM} (\textit{Gray-Level Co-occurrence Matrix}) ou matrice de co-occurrences a été proposée par Haralick et al. \cite{ref_03}. Elle permet d'analyser les textures en calculant les répartitions des différents niveaux de gris dans l'image en se basant sur les positions relatives des pixels.\vspace{0.5em}  

	Il est possible de l'utiliser directement et appliquer la \textit{réduction de dimensionnalité} ou bien d'extraire différentes caractéristiques comme le contraste, l'énergie, l'homogénéité, etc.
	...
	Le $k$-\textbf{NN} consiste à trouver les $k$ échantillons à partir de la base de données de référence qui se rapprochent le plus de l'échantillon d'entrée. Le critère utilisé est la distance de \textit{Manhattan}. C'est sur la base d'un vote de majorité sur les $k$ échantillons que l'identité est déterminée.
