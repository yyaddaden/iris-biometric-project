\block{Méthodologie d'évaluation}{
	...
	\begin{itemize}
		...
		\item[$\rightarrow$] Caractéristiques biométriques pour $\mathbf{46}$ personnes, mais uniquement $\mathbf{10}$ sélectionnés aléatoirement ont été utilisés.
		
		\item[$\rightarrow$] Durant les évaluation, c'est \textit{validation croisée} qui a été adoptée. Elle permet de générer $\mathbf{K = 10}$ parties \textit{stratifiés} afin d'éviter les biais.
	\end{itemize}	 	
	}
}
